function PlotTruss(model,U,n,C)
%% Plot
U=U(:,n)/max(U(:,n))*0.2;
Coor=model.Coor;
Conn=model.Conn;
...
for i=1:ne
    ...
    x1=Coor(node1,1)+U(3*node1-2)*ScaleFactor;
    x2=Coor(node2,1)+U(3*node2-2)*ScaleFactor;
    y1=Coor(node1,2)+U(3*node1-1)*ScaleFactor;
    y2=Coor(node2,2)+U(3*node2-1)*ScaleFactor;   
    z1=Coor(node1,3)+U(3*node1)*ScaleFactor;
    z2=Coor(node2,3)+U(3*node2)*ScaleFactor;
